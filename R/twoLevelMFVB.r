########## R function: twoLevelMFVB ##########

# For streamlined mean field variational Bayes
# fitting of two-level models.

# Last changed: 20 FEB 2014
##############################################

twoLevelMFVB <- function(y,XG,XR,ZG,ZR=NULL,reBlockInds,ncZG,
...
{    
   ...
   if (responseType=="Bernoulli")
   {
      ...
      phi <- function(x)
             (x/2) - log(1+exp(x)) + (0.25*x*tanh(x/2))
   }
   
   # Obtain constant matrices and dimension variables:
  
   X <- cbind(XR,XG); ncX <- ncol(X); ncXR <- ncol(XR)
   CG <- cbind(X,ZG); L <- length(ncZG)
   ncCG <- ncol(CG)
                     ...
                     mu.q.recip.a.eps,mu.q.recip.sigsq.eps)
   {
      ...
      ans0 <- (0.5*ncXR*ASigma*log(2*nuVal) - 0.5*numObs*log(2*pi) 
      ...
      if (responseType=="Bernoulli")
      {
         ...
         return(list(ans0,ans2,ans3,ans4,ans5,ans6,ans7,ans8,ans9,ans10,ans))
      }
   }
   ...
   while (!converged) 
   {
      ...
      {      
         ...
         for (i in 1:m)
         {
            ...
            mu.q.uR[[i]] <- as.vector(mu.q.recip.sigsq.eps*(zetaVec[[i]] 
                            + H[[i]]%*%(kappaVec[[i]] 
                            - crossprod(G[[i]],Sigma.q.betauG%*%CGTy))))
         }
         ...
         detBMat <- matrix(0,ncCG,ncCG); indsStt <- 1
         for (i in 1:m)
         {
      ...
      if (responseType=="Bernoulli")
      {
         ...
         if (doStreamlined)
         {
            ...
            if (!useMatForDg)
            {
               ...
               for (i in 1:sum(nVec))
                  xiSqd[i] <- crossprod(CG[i,],(Sigma.q.betauG + tcrossprod(mu.q.betauG)))%*%CG[i,]
            }

            for (i in 1:m)
            {
               EsqMatCurr <- (-Sigma.q.betauG)%*%G[[i]]%*%H[[i]] + tcrossprod(mu.q.betauG,mu.q.uR[[i]])
   ...
   {
      if (responseType=="Gaussian")
         return(list(mu.q.betauG=mu.q.betau[1:ncCG],Sigma.q.betauG=Sigma.q.betau[1:ncCG,1:ncCG],
                     A.q.sigsq.eps=A.q.sigsq.eps,B.q.sigsq.eps=B.q.sigsq.eps,logMLgrid=logMLgrid))
      
      if (responseType=="Bernoulli")
         return(list(mu.q.betauG=mu.q.betau[1:ncCG],Sigma.q.betauG=Sigma.q.betauG[1:ncCG,1:ncCG],
                     logMLgrid=logMLgrid))
   }
   ...
   if (doStreamlined)
   {
      ...
      if (responseType=="Bernoulli")
         return(list(mu.q.betauG=mu.q.betauG,Sigma.q.betauG=Sigma.q.betauG,logMLgrid=logMLgrid))
   }
}   
